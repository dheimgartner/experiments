...
replace_effect_codes <- function(design, add_units = TRUE, lang = c("en", "de")) {
  ...
  labels_helper <- function(lang = c("en", "de"), labels) {
    lang <- match.arg(lang)
    if (lang == "en") {
      return(labels$en)
    } else if (lang == "de") {
      ...
      stop("Only en and de implemented")
    }
  }
  ...
  lh <-
    labels_helper(lang = lang,
                  labels = list(en = c("Up to 25 km/h", "Up to 45 km/h"),
                                de = c("Bis zu 25 km/h", "Bis zu 45 km/h")))
  design <-
    design %>%
    sub_codes("eb_cost", "eb_type", lh, c(50, 80))

  lh <-
    labels_helper(lang = lang,
                  labels = list(en = "Modular Travelcard (fare networks)",
                                de = "Modul-Abo (Verbund)"))

  ## price of modulabo depends on zones included
  design <-
    design %>%
    dplyr::mutate(
      A_ma = ifelse(A_pt_type == lh, paste(A_pt_type, A_pt_zones, sep = "_"), as.character(A_pt_type)),
      B_ma = ifelse(B_pt_type == lh, paste(B_pt_type, B_pt_zones, sep = "_"), as.character(B_pt_type))
    )

  lh <-
    labels_helper(lang = lang,
                  labels = list(en = c("GA", "Half Fare", "Modular Travelcard (fare networks)_1-2 zones", "Modular Travelcard (fare networks)_Whole fare network"),
                                de = c("GA", "Halbtax", "Modul-Abo (Verbund)_1-2 Zonen", "Modul-Abo (Verbund)_Ganzer Tarifverbund")))

  design <-
    design %>%
    sub_codes("pt_fix_cost", "ma", lh, c(300, 15, 65, 185))
  ...
  lh <-
    labels_helper(lang = lang,
                  labels = list(en = c("GA", "Half Fare", "Modular Travelcard (fare network)"),
                                de = c("GA", "Halbtax", "Modul-Abo (Verbund)")))

  design <-
    design %>%
    sub_codes("pt_variable_cost", "pt_type", lh, c(NA_real_, 0.5, NA_real_))
